...
**Two implementations, one matrix** (call-core plan, D5 option (a)): a row may also
carry `RUST:<path>::<fn>` — `path` relative to `core/call-core`, a `#[test] fn`
defined in that file. `core/call-core/tests/enforcement_matrix.rs` walks the
`RUST:` pins the way `EnforcementMatrixTest` walks the `PIN:` pins; each side
checks only its own. A row pinned on the Kotlin side only passes, but the COUNT
of such rows is asserted in the Rust walker and may only shrink — incomplete
coverage stays visible instead of silent. Rows that only a journey or the shell
can pin (real ports, `MainViewModel`, the screen) stay one-sided by design until
Android is on the core (D3).
...
|---|---|---|
| Signed in, device published | MUST CAMP WITH PASS (silently) | PIN:CampAdmissionTest.kt::`a mint camps with the token and caches it by the wire's expiresIn` · arc PIN:CampJourneyTest.kt::`oauth session to camping pass to expiry re-mint — the full arc` — **DEVICE-VERIFIED 2026-09-08** (runbook §15): both physical phones earned `admitted endpoint_id=… sponsorship=BudgetBytes(262144)` on the ENFORCING production relay, each through the full arc (attach → `denied reason="no_token"` → mint → admitted), with the screen reading "ready, camped on relay" and the negative state — a genuinely refused attach reading "NOT camped" — observed on the same hardware in the same session. The callee held one admitted connection for 14 minutes with no further verdicts. Prior E153 history: production silently ran relay v0.1.1 through a deploy-guard bug and refused every pass until 2026-08-30; the §13 "live" citation rested on attributed `usage` lines, which prove presentation, not admission. · RUST:tests/camp_rows.rs::a_mint_camps_with_the_token_and_caches_it_by_the_wires_expires_in |
| Signed out | MUST DEGRADE (tokenless, silent — v0.4.0 shape) | PIN:CampAdmissionTest.kt::`signed-out camps tokenless with no note` · RUST:tests/camp_rows.rs::signed_out_camps_tokenless_with_no_note |
| Signed in, no cached pass | MUST MINT | PIN:CampAdmissionTest.kt::`signed-in with no cached pass mints` · RUST:tests/camp_rows.rs::signed_in_with_no_cached_pass_mints |
| Live cached pass | MUST REUSE (the token is the cache) | PIN:CampAdmissionTest.kt::`a live cached pass is reused — the token is the cache` · RUST:tests/camp_rows.rs::a_live_cached_pass_is_reused_the_token_is_the_cache |
| Pass near expiry | MUST RE-MINT (margin, boundary exact) | PIN:CampAdmissionTest.kt::`a pass inside the re-mint margin mints fresh instead of riding expiry` · PIN:CampAdmissionTest.kt::`a pass exactly at the margin boundary still mints` · RUST:tests/camp_rows.rs::a_pass_inside_the_remint_margin_mints_fresh · RUST:tests/camp_rows.rs::a_pass_exactly_at_the_margin_boundary_still_mints · RUST:tests/camp_rows.rs::a_pass_one_past_the_margin_boundary_is_reused |
| Admit refuses the camp (endpoint_unbound, …) | MUST DEGRADE WITH WORDS | PIN:CampAdmissionTest.kt::`a refusal camps tokenless with words — reception must not die quietly` · PIN:CampAdmissionTest.kt::`each refusal reason has its own words` · observed live at production 2026-08-26 02:23Z · RUST:tests/camp_rows.rs::a_refusal_camps_tokenless_with_words · RUST:tests/camp_rows.rs::each_refusal_reason_has_its_own_words |
...
| Admit outage | MUST DEGRADE WITH availability note | PIN:CampAdmissionTest.kt::`an outage camps tokenless with the availability note` · PIN:CampJourneyTest.kt::`an admit outage camps tokenless with the availability note` · RUST:tests/camp_rows.rs::an_outage_camps_tokenless_with_the_availability_note |
| Client defect (bad request) | MUST DEGRADE AND SAY SO | PIN:CampAdmissionTest.kt::`a client defect camps tokenless and says so` · RUST:tests/camp_rows.rs::a_client_defect_camps_tokenless_and_says_so |
| Sign-out | MUST DROP THE PASS | PIN:CampAdmissionTest.kt::`signing out drops the pass — a cached pass without a session does not camp` · §12 sign-out rung · RUST:tests/camp_rows.rs::signing_out_drops_the_pass |

## Dial posture (M4c — refusals never dial; outages dial tokenless)

| Scenario | Outcome | Pinned by |
|---|---|---|
| Ticket secret on the card | MUST DIAL with minted token (possession proof) | PIN:DialAdmissionTest.kt::`a ticket secret is the proof whenever the card carries one` · PIN:DialCompositionJourneyTest.kt::`ticket in hand dials with a real minted token` · PIN:TicketJourneyTest.kt::`invite link to relay token, end to end over real HTTP` · RUST:tests/dial_rows.rs::a_ticket_secret_is_the_proof_whenever_the_card_carries_one |
| Signed in, no secret | MUST DIAL with identity proof | PIN:DialAdmissionTest.kt::`a signed-in caller proves identity when there is no secret` · PIN:IdentityJourneyTest.kt::`oauth session to service-auth proof to relay token` · RUST:tests/dial_rows.rs::a_signed_in_caller_proves_identity_when_there_is_no_secret |
| Minted token | MUST DIAL WITH IT (EndpointId stable) | PIN:DialAdmissionTest.kt::`a minted token dials with it` · RUST:tests/dial_rows.rs::a_minted_token_dials_with_it |
| Admit REFUSES (revoked, mismatch, …) | MUST REFUSE — never dials, words on screen | PIN:DialAdmissionTest.kt::`every refusal blocks the dial with its own honest message` · PIN:DialCompositionJourneyTest.kt::`a revoked grant refuses and nothing dials` · PIN:TicketJourneyTest.kt::`a grant deleted after redeem yields no token at mint time` · RUST:tests/dial_rows.rs::every_refusal_blocks_the_dial_with_its_own_honest_message |
...
| Admit outage | MUST DEGRADE — dials tokenless AND SAYS SO | PIN:DialAdmissionTest.kt::`an admit outage dials tokenless and says so` · PIN:DialCompositionJourneyTest.kt::`an admit outage dials tokenless and says so` · RUST:tests/dial_rows.rs::an_admit_outage_dials_tokenless_and_says_so |
| v1 callee (no grant params) | MUST DIAL tokenless, silently (compat) | PIN:DialAdmissionTest.kt::`no grant means the v1 tokenless dial, silently` · RUST:tests/dial_rows.rs::no_grant_means_the_v1_tokenless_dial_silently — **DEVICE-VERIFIED 2026-09-14/21/23** (runbook §16, §17): every phone dial in this arc was a `croftcall://call?endpoint=…&relay=…&handle=…` card with no grant params — Pixel→arc and arc→Pixel (§17, 2026-09-21), Samsung↔Pixel both directions (§17, the same evening), Pixel on LTE↔Samsung both directions (§17, 2026-09-23) — and each dialled tokenless with no mint, no words on screen, and no line in the relay journal, then connected. The negative state on the same hardware: the one tokenless dial that failed (Samsung→backgrounded Pixel, 22:07:20) refused with words — "no answer within 20s" — silence never hid a failure. A grant-bearing dial has not been placed from a phone, so the token rows above stay unmarked. |
| Grant but no usable proof | MUST DEGRADE with a sign-in nudge, mint untouched | PIN:DialAdmissionTest.kt::`a grant with no usable proof dials tokenless with a sign-in nudge` · PIN:DialCompositionJourneyTest.kt::`signed out with no secret nudges and never touches the mint` · RUST:tests/dial_rows.rs::a_grant_with_no_usable_proof_dials_tokenless_with_a_sign_in_nudge |
| Client defect (bad request) | MUST REFUSE and say it is ours | PIN:DialAdmissionTest.kt::`a bad request is a client defect and blocks the dial` · RUST:tests/dial_rows.rs::a_bad_request_is_a_client_defect_and_blocks_the_dial |
| Dialling while holding a live camping pass | MUST KEEP the pass — a dial never lowers admission | PIN:RebindPolicyTest.kt::`a tokenless dial keeps a live camping pass — reachability is never lowered` · PIN:RebindPolicyTest.kt::`the same token is not a rebind` — **DEVICE-VERIFIED 2026-09-14** (runbook §16): one Connect tap on a real phone, and the relay journal carried **one line in total** from the tap through the connected call and the hang-up — no `Stream terminated`, no `usage` close, no denial, no re-admit. §15.3's identical action produced all four within a second. Held again over our own port (§17): no relay line for either phone through both directions on the LAN (2026-09-21) and through the relayed-then-direct calls with the Pixel on LTE (2026-09-23) — a re-admit only when a phone came back from background, with its remembered pass. · RUST:tests/rebind_rows.rs::a_tokenless_dial_keeps_a_live_camping_pass · RUST:tests/rebind_rows.rs::the_same_token_is_not_a_rebind · RUST:tests/rebind_rows.rs::tokenless_while_already_tokenless_is_not_a_rebind |
| Dialling with a minted call token | MUST SWAP (M4c), accepting the camp gap | PIN:RebindPolicyTest.kt::`a minted call token swaps — a different admitting token is the M4c path` · PIN:RebindPolicyTest.kt::`an unbound endpoint binds the minted token` · RUST:tests/rebind_rows.rs::a_minted_call_token_swaps · RUST:tests/rebind_rows.rs::an_unbound_endpoint_binds_the_minted_token |
