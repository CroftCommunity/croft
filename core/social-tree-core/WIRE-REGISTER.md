...
| `GroupState` | **0x03** (`model::GROUP_STATE_WIRE_VERSION`) | `model.rs` (`to_bytes`/`from_bytes`) | v3 = v2 + the §7.6.4 standing-ceiling set (banned lineages, count ‖ 32 each, appended after fork status) — P4's admission facts need bans readable at position; v1/v2 refused with a rebuild demand |
