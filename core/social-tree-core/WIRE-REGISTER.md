...
| `GroupState` | **0x04** (`model::GROUP_STATE_WIRE_VERSION`) | `model.rs` (`to_bytes`/`from_bytes`) | v4 = v3 + the sixth threshold (`readmission_threshold`, u32 after resolution — rule key 5, the un-ban dial, owner decision 2026-08-25); v3 = v2 + the §7.6.4 standing-ceiling set (banned lineages, appended after fork status); v1–v3 refused with a rebuild demand |
